...
def gen_html(data, config):
    ...
    html += '/* ─── 搜索筛选函数 ─── */\n'
    html += 'function af(){var yr=document.getElementById("yrf").value;var jr=document.getElementById("jrf").value;document.querySelectorAll(".pi").forEach(function(pi){var py=pi.getAttribute("data-year")||"";var pj=pi.getAttribute("data-jrank")||"";pi.dataset.afHide=(yr!=="all"&&py!==yr)||(jr!=="all"&&pj!==jr)?"1":""});ps()}\n'
    html += 'function ps(){var q=document.getElementById("ps").value.toLowerCase().trim();document.querySelectorAll(".pi").forEach(function(pi){var t=(pi.querySelector(".pi-t a")||{}).textContent||"";t=t.toLowerCase();var m=(pi.querySelector(".pi-m")||{}).textContent||"";m=m.toLowerCase();var hide=!!pi.dataset.afHide||(q&&!t.includes(q)&&!m.includes(q));pi.style.display=hide?"none":""});var v=document.querySelectorAll(".pi:not([style*=none])").length;document.getElementById("rc").textContent=v>0?"找到 "+v+" 篇":""}\n'
    html += '/* ─── 文献收藏 ─── */\n'
